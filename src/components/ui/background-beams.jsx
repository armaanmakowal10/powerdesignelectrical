...
/** Pushes beam artwork down inside the viewBox (SVG user units). */
const BEAM_VERTICAL_NUDGE = 92;
...
export const BackgroundBeams = React.memo(function BackgroundBeams({ className }) {
  ...
  return (
    <div className={cn('pointer-events-none absolute inset-0 h-full w-full', className)}>
      ...
      >
        <g transform={`translate(0, ${BEAM_VERTICAL_NUDGE})`}>
          <g opacity="0.03">
            {pathData.map((d, i) => (
